...
#Load Gene
stad_gene.df <- read.delim('./stad_metamap/conceptsInFile_Gene.txt',sep = '\t')
...
#Output path
output_path <- './output/stad_metamap/'
...
stad.tdf$month = tmdf$month
#ZIPF_law filter count 1 concept
stad_total_concept.tdf <- stad.tdf %>%
  count(Concept)
index <- stad_total_concept.tdf[stad_total_concept.tdf$n>1,]$Concept
...
abs_word_pairs_cosine <- stad.tdf %>%
  #filter(Count >5) %>%
