...
#Load Gene
lihc_gene.df <- read.delim('./lihc_metamap/conceptsInFile_Gene.txt',sep = '\t')
...
#Output path
output_path <- './output/lihc_metamap/'
...
lihc.tdf$month = tmdf$month
#ZIPF_law filter count 1 concept
lihc_total_concept.tdf <- lihc.tdf %>%
  count(Concept)
index <- lihc_total_concept.tdf[lihc_total_concept.tdf$n>1,]$Concept
...
abs_word_pairs_cosine <- lihc.tdf %>%
  #filter(Count >5) %>%
