# -*- coding: utf-8 -*-
"""
Created on Mon Mar 12 12:57:23 2018
...
def outputConceptsInFile2(conceptsInFile,date,outpath,option='term'):
    #option 可以为 'cui' 或 'term', cui表示输出概念的唯一id, term表示输出概念的首选词
    category_list=cf.CATEGORYs
    ...
    #格式：filenames	0date	0concept_num	及各个category下的concept
    file=open(outpath,'w')
    file.write('#filename\tdate\tconcept_num\tconcept'+'\n')
    for filename in conceptsInFile:
        d=date[filename]
        conceptslist=conceptsInFile[filename]
        for CUI in conceptslist:
            CountInFile=conceptslist[CUI]['count']
            SemanticType=conceptslist[CUI]['semanticType']
            Concept=conceptslist[CUI]['concept']
            Category=conceptslist[CUI]['category']
            line=filename+'\t'+d+'\t'+Concept+'\t'+str(CountInFile)+'\n'
            #print(line)
            file.write(line)
...
def main(metamapRe_dir,generalConcepts_path,generalStrings_path,targetSemanticType_path,output_dir,date_path='..\\knol\\da.txt'): 
    conceptSummary_path=os.path.join(output_dir,'conceptSummary.txt')
    conceptsInFile_path=os.path.join(output_dir,'conceptsInFile2.txt')
    ...
    outputConceptsInFile2(conceptsInFile,date,conceptsInFile_path)
